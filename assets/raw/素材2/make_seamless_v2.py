import os
import numpy as np
from PIL import Image

K = 64  # 边缘融合带宽

def edge_diff(arr):
    h, w, _ = arr.shape
    left = arr[:, 0, :].astype(np.int16)
    right = arr[:, w - 1, :].astype(np.int16)
    return float(np.mean(np.abs(left - right)))

def edge_fuse_unit(path_in, path_out, preview_out, unit_w=1024, k=K):
    img = Image.open(path_in).convert("RGB")
    arr = np.array(img)
    h, w, _ = arr.shape
    x0 = (w - unit_w) // 2
    crop = arr[:, x0:x0 + unit_w, :].copy()
    before = edge_diff(crop)
    new = crop.copy()
    mid = ((crop[:, 0, :].astype(np.int16) + crop[:, -1, :].astype(np.int16)) // 2)
    for x in range(k):
        t = x / k
        new[:, x, :] = (mid * (1 - t) + crop[:, x, :].astype(np.int16) * t).astype(np.uint8)
        new[:, -1 - x, :] = (mid * (1 - t) + crop[:, -1 - x, :].astype(np.int16) * t).astype(np.uint8)
    after = edge_diff(new)
    Image.fromarray(new).save(path_out, optimize=True)
    # 平铺预览：单元并排 3 次
    tile = np.concatenate([new, new, new], axis=1)
    Image.fromarray(tile).save(preview_out, optimize=True)
    return unit_w, before, after

base = r"C:\Users\Administrator\Desktop\素材2\背景"
jobs = [
    ("01洪崖洞-现代夜", "中景"), ("01洪崖洞-现代夜", "地形"),
    ("02磁器口-古代窑场", "中景"), ("02磁器口-古代窑场", "地形"),
    ("03中山古镇-古代", "中景"), ("03中山古镇-古代", "地形"),
    ("04防空洞-近代", "中景"), ("04防空洞-近代", "地形"),
    ("05洪崖洞-归来晨光", "中景"), ("05洪崖洞-归来晨光", "地形"),
]

print("文件 | 尺寸 | 边缘差异(前) | 边缘差异(后) | 提升")
for folder, layer in jobs:
    fin = os.path.join(base, folder, f"{layer}_循环背景_v2.jpg")
    fout = os.path.join(base, folder, f"{layer}_无缝单元_1024_v2.png")
    pout = os.path.join(base, folder, f"{layer}_平铺预览_v2.png")
    w, before, after = edge_fuse_unit(fin, fout, pout)
    pct = (1 - after / before) * 100 if before > 0 else 0
    print(f"{folder}/{layer} | {w}px | {before:.2f} | {after:.2f} | {pct:.1f}%")
